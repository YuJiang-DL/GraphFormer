...
# def new_Whole_supernode_vis(WSI, Position_pd, Location_pd, Select_ID, core_dir, edge_mask_norm, row, col):
#     """
#     在完整WSI上绘制解释图（Integrated Gradients叠加），
#     使用半透明、柔和色阶、提高分辨率输出。
#     """
#
#     WSI_level = 1
#     print(f"WSI.level_downsamples: {WSI.level_downsamples}")
#     print(f"Trying to access level {WSI_level}")
#
#     Downsample_ratio = int(WSI.level_downsamples[WSI_level])
#     WSI_width, WSI_height = WSI.level_dimensions[WSI_level]
#
#     # 读取低倍的 WSI 原图
#     WSI_image = WSI.read_region((0, 0), WSI_level, (WSI_width, WSI_height)).convert('RGB')
#     WSI_image_np = np.array(WSI_image)
#
#     Image_IG_mask = np.zeros((WSI_height, WSI_width), dtype=np.float32)
#     Image_IG_mask_count = np.zeros((WSI_height, WSI_width), dtype=np.float32)
...
#         Superpatch = int(Superpatch_node_row[2])
...
#
#         # 超级节点
#         y_start, x_start = Superpatch_pos['Y'] * WSI_patch_dimension, Superpatch_pos['X'] * WSI_patch_dimension
#         Image_IG_mask[y_start:y_start + WSI_patch_dimension, x_start:x_start + WSI_patch_dimension] += edge_mask_norm[
#             col_idx]
#         Image_IG_mask_count[y_start:y_start + WSI_patch_dimension, x_start:x_start + WSI_patch_dimension] += 1
#
#         # 邻域节点
#         for Env_y, Env_x in zip(Envpatch_pos['Y'], Envpatch_pos['X']):
#             y_env, x_env = Env_y * WSI_patch_dimension, Env_x * WSI_patch_dimension
#             Image_IG_mask[y_env:y_env + WSI_patch_dimension, x_env:x_env + WSI_patch_dimension] += edge_mask_norm[
#                 col_idx]
#             Image_IG_mask_count[y_env:y_env + WSI_patch_dimension, x_env:x_env + WSI_patch_dimension] += 1
#
#     # 平均解释值
#     valid_mask = Image_IG_mask_count > 0
#     Image_IG_mask[valid_mask] /= Image_IG_mask_count[valid_mask]
#
#     # 归一化热图
#     Image_IG_mask = Image_IG_mask - np.min(Image_IG_mask)
#     if np.max(Image_IG_mask) > 0:
#         Image_IG_mask = Image_IG_mask / np.max(Image_IG_mask)
#
#
#     # ===== 平滑：让patch块状边界变柔和 =====
#     # sigma 越大越平滑，建议 2~8 之间试（level=1 通常 3~6 很合适）
#     # Image_IG_mask = cv.GaussianBlur(Image_IG_mask, ksize=(0, 0), sigmaX=4, sigmaY=4)
#     # # 将解释值映射到颜色（colormap）
#     # heatmap = (cm.plasma(Image_IG_mask)[:, :, :3] * 255).astype(np.uint8)
#     #
#     # # 将热图和原图叠加（增加透明度，alpha混合）
#     # overlay = (0.4 * WSI_image_np + 0.6 * heatmap).astype(np.uint8)
#     # overlay_image = Image.fromarray(overlay)
#
#     # 将解释值映射到颜色（colormap）
#     # heatmap_rgba = cm.plasma(Image_IG_mask)  # RGBA格式
#     heatmap_rgba = cm.coolwarm(Image_IG_mask)  # 或 cm.seismic
#     # 创建叠加图，初始化为原图
#     overlay = WSI_image_np.copy()
#
#     # 找到有解释值（非零）的区域
#     mask_nonzero = Image_IG_mask > 0
#
#     # 仅在有解释值的位置进行混合叠加
#     overlay[mask_nonzero] = (
#             0.5 * WSI_image_np[mask_nonzero] +
#             0.5 * (heatmap_rgba[mask_nonzero][:, :3] * 255)
#     ).astype(np.uint8)
#
#     # 转换为Image对象并保存
#     overlay_image = Image.fromarray(overlay)
...
#     save_path = os.path.join(core_dir, f"{Select_ID}_WSI_IG_overlay.png")
#     overlay_image.save(save_path, dpi=(300, 300))  # 提高dpi，保存为高质量PNG
#
...
# def new_Whole_supernode_vis(WSI, Position_pd, Location_pd, Select_ID, core_dir, edge_mask_norm, row, col):
#     """
#     在完整WSI上绘制解释图（IG/attention叠加）
#     - 你的原逻辑：每个patch铺块 -> sum/count -> 归一化 -> colormap -> alpha混合
#     - 我加的：坐标转int+边界裁剪；归一化只在有效区域；mask用count>0避免全图染色；可选blur
#     """
#
#     # -------------------------
#     # 1) 读 WSI
...
#     print(f"Trying to access level {WSI_level}")
#
#     Downsample_ratio = float(WSI.level_downsamples[WSI_level])
#     WSI_width, WSI_height = WSI.level_dimensions[WSI_level]
#
#     WSI_image = WSI.read_region((0, 0), WSI_level, (WSI_width, WSI_height)).convert('RGB')
#     WSI_image_np = np.array(WSI_image).astype(np.float32)
#
#     # patch 在该 level 下的边长
#     WSI_patch_dimension = int(round(256 / Downsample_ratio))
#     WSI_patch_dimension = max(1, WSI_patch_dimension)
#
#     Image_IG_mask = np.zeros((WSI_height, WSI_width), dtype=np.float32)
#     Image_IG_mask_count = np.zeros((WSI_height, WSI_width), dtype=np.float32)
#
...
#
#         Envpatch = Superpatch_node_row[3:].to_numpy().astype(int)
#         Envpatch_pos = Location_pd.iloc[Envpatch] if Envpatch.size > 0 else None
#
#         val = float(edge_mask_norm[col_idx])
#
#         # ---- 超级节点 ----
#         y_start = int(Superpatch_pos['Y'] * WSI_patch_dimension)
#         x_start = int(Superpatch_pos['X'] * WSI_patch_dimension)
#         y_end = min(WSI_height, y_start + WSI_patch_dimension)
#         x_end = min(WSI_width,  x_start + WSI_patch_dimension)
#
#         if y_start < WSI_height and x_start < WSI_width and y_end > 0 and x_end > 0:
#             y0 = max(0, y_start); x0 = max(0, x_start)
#             Image_IG_mask[y0:y_end, x0:x_end] += val
#             Image_IG_mask_count[y0:y_end, x0:x_end] += 1.0
#
#         # ---- 邻域节点 ----
#         if Envpatch_pos is not None:
#             for Env_y, Env_x in zip(Envpatch_pos['Y'], Envpatch_pos['X']):
#                 y_env = int(Env_y * WSI_patch_dimension)
#                 x_env = int(Env_x * WSI_patch_dimension)
#                 y_end = min(WSI_height, y_env + WSI_patch_dimension)
#                 x_end = min(WSI_width,  x_env + WSI_patch_dimension)
#
#                 if y_env < WSI_height and x_env < WSI_width and y_end > 0 and x_end > 0:
#                     y0 = max(0, y_env); x0 = max(0, x_env)
#                     Image_IG_mask[y0:y_end, x0:x_end] += val
#                     Image_IG_mask_count[y0:y_end, x0:x_end] += 1.0
#
#     # -------------------------
#     # 3) 平均解释值：sum/count
#     # -------------------------
#     valid = Image_IG_mask_count > 0
#     if np.any(valid):
#         Image_IG_mask[valid] = Image_IG_mask[valid] / Image_IG_mask_count[valid]
#     else:
#         # 没有任何有效区域，直接保存原图
#         overlay_image = Image.fromarray(WSI_image_np.astype(np.uint8))
#         os.makedirs(core_dir, exist_ok=True)
#         save_path = os.path.join(core_dir, f"{Select_ID}_WSI_IG_overlay.png")
#         overlay_image.save(save_path, dpi=(300, 300))
#         return overlay_image
#
#     # -------------------------
#     # 4) 归一化：只在 valid 区域做（避免背景影响）
#     # -------------------------
#     vmin = Image_IG_mask[valid].min()
#     vmax = Image_IG_mask[valid].max()
#     Image_IG_mask = Image_IG_mask - vmin
#     if (vmax - vmin) > 1e-12:
#         Image_IG_mask = Image_IG_mask / (vmax - vmin)
#     else:
#         Image_IG_mask[:] = 0.0
#
#     # -------------------------
...
#     # -------------------------
#     # 如果你想更柔和：取消注释这一行
#     # Image_IG_mask = cv.GaussianBlur(Image_IG_mask, (0, 0), sigmaX=4, sigmaY=4)
#
#     # -------------------------
#     # 6) colormap + overlay（权重相加=1）
#     # -------------------------
#     heatmap_rgba = cm.coolwarm_r(Image_IG_mask)  # [H,W,4] 0..1
#
#     overlay = WSI_image_np.copy()
#
#     # 叠加区域：用 count>0 更合理（避免 min-max 后 >0 变成全图）
#     mask_nonzero = valid
#
#     hm = (heatmap_rgba[:, :, :3] * 255.0).astype(np.float32)
#
#     overlay[mask_nonzero] = (
#         0.2 * WSI_image_np[mask_nonzero] +
#         0.8 * hm[mask_nonzero]
#     )
#
...
#     # -------------------------
#     os.makedirs(core_dir, exist_ok=True)
#     save_path = os.path.join(core_dir, f"{Select_ID}_WSI_IG_overlay_-1.png")
#     overlay_image.save(save_path, dpi=(300, 300))
#
#     return overlay_image
...
def new_Whole_supernode_vis(
    WSI, Position_pd, Location_pd, Select_ID, core_dir,
    edge_mask_norm, row, col,
    WSI_level=2,
    alpha=0.7,          # 热图叠加透明度（越大越“热”）
    blur=True,           # 是否开启平滑
    overlap=0.0,         # 0~1：只影响模糊核半径（类似CLAM里的overlap参数语义）
    blur_on_rgb=False,   # 是否对最终RGB再做一次blur（更柔，但会糊底图）
    custom_sigma=None    # 如果你想手动指定sigma，传入比如 3~8
):
    """
    在完整WSI上绘制解释图（IG/attention叠加），并产生 CLAM 风格“平滑热图”效果：
    - sum/count -> normalize -> (可选) GaussianBlur(mask) -> colormap -> alpha blend（软边缘）
    """
    ...
    # 4) 归一化：只在 valid 区域做（避免背景影响）
    # -------------------------
    vmin = float(Image_IG_mask[valid].min())
    vmax = float(Image_IG_mask[valid].max())
    Image_IG_mask = Image_IG_mask - vmin
    if (vmax - vmin) > 1e-12:
        Image_IG_mask = Image_IG_mask / (vmax - vmin)
    else:
    ...
    # -------------------------
    # 5) CLAM风格平滑：mask本身 & valid边缘软化
    # -------------------------
    if blur:
        # 参考 CLAM：kernel 大小与 patch_size*(1-overlap) 相关
        # 这里 patch_size 就用 WSI_patch_dimension
        k = int(max(1, round(WSI_patch_dimension * (1.0 - float(overlap)))))
        k = k * 2 + 1  # 保证奇数
        k = max(3, k)  # 至少3

        # ① 平滑解释强度图（让块状变连续）
        if custom_sigma is None:
            Image_IG_mask = cv.GaussianBlur(Image_IG_mask, (k, k), 0)
        else:
            # sigma 手动控制更直观：3~8通常就很“丝滑”
            Image_IG_mask = cv.GaussianBlur(Image_IG_mask, (0, 0), float(custom_sigma))

        # ② 软化 valid 边缘：做一个“透明度地图”而不是硬mask
        valid_f = valid.astype(np.float32)
        if custom_sigma is None:
            alpha_map = cv.GaussianBlur(valid_f, (k, k), 0)
        else:
            alpha_map = cv.GaussianBlur(valid_f, (0, 0), float(custom_sigma))

        # 归一化到 0~1，避免扩散后 >1
        mx = alpha_map.max()
        if mx > 1e-6:
            alpha_map = alpha_map / mx
    ...
    # -------------------------
    # 6) colormap + alpha blend（软边缘、平滑叠加）
    # -------------------------
    heatmap_rgba = cm.coolwarm(Image_IG_mask)  # [H,W,4] 0..1
    # CLAM JET NOT COOLWARM
    # heatmap_rgba = cm.jet(Image_IG_mask)  # [H,W,4] 0..1
    hm = (heatmap_rgba[:, :, :3] * 255.0).astype(np.float32)

    overlay = WSI_image_np.copy()

    # 最终每个像素的混合权重：alpha * alpha_map
    a = float(alpha) * alpha_map  # [H,W] in [0, alpha]

    # 广播到3通道做融合：overlay = (1-a)*orig + a*hm
    overlay = overlay * (1.0 - a[..., None]) + hm * (a[..., None])

    overlay = np.clip(overlay, 0, 255).astype(np.uint8)

    if blur_on_rgb and blur:
        # 可选：对最终RGB再柔化一次（更像“雾化热图”）
        overlay = cv.GaussianBlur(overlay, (0, 0), 1.2)

    overlay_image = Image.fromarray(overlay)
    # image = Image.fromarray(np.clip(hm, 0, 255).astype(np.uint8))
    heatmap_rgb = np.clip(hm, 0, 255).astype(np.uint8)
    heatmap_alpha = (alpha_map * 255).astype(np.uint8)
    # -------------------------
    # 7) 保存
    # -------------------------
    os.makedirs(core_dir, exist_ok=True)
    save_path = os.path.join(core_dir, f"{Select_ID}_WSI_IG_overlay_smooth.png")
    save_path2 = os.path.join(core_dir, f"{Select_ID}_WSI_IG.png")
    overlay_image.save(save_path, dpi=(300, 300))
    heatmap_rgba_save = np.dstack([heatmap_rgb, heatmap_alpha])
    heatmap_image = Image.fromarray(heatmap_rgba_save, mode='RGBA')
    heatmap_image.save(save_path2, dpi=(300, 300))
...
def Whole_supernode_vis(WSI, Position_pd, Location_pd, Select_ID, core_dir, edge_mask_norm, row, col):
    ...
    # 依次迭代在Position_pd中找到的每一行，并将该行中的位置信息提取出来
    for col_idx in range(Position_pd.shape[0]):
        # 使用dropna函数删除在Superpatch_node_row（一个包含位置信息的Pandas系列）中的任何缺失值
        Superpatch_node_row = Position_pd.iloc[col_idx].dropna()
        # 将Superpatch_node_row中的第三个位置信息转换为整数，并将其存储在Superpatch中
        Superpatch = int(Superpatch_node_row[2].tolist())
        # 使用值为Superpatch的位置信息从Location_pd中检索位置，并将其存储在Superpatch_pos中
        Superpatch_pos = Location_pd.iloc[Superpatch]
        # 将Superpatch_node_row中的其他位置信息转换为整数，并将其存储在一个名为Envpatch的NumPy数组中
        Envpatch = Superpatch_node_row[3:].to_numpy().astype(int)
        # 对于Envpatch中的每个值，使用值作为索引从Location_pd中检索位置，并将这些位置存储在一个名为Envpatch_pos的Pandas数据帧中
        Envpatch_pos = Location_pd.iloc[Envpatch]

        # 通过 Superpatch_pos 字典提供的位置以及超级补丁的维度，将在 Image_mask 数组中指定的区域内的元素的值设置为 2
        ...
        # 通过 Superpatch_pos 索引从 edge_mask_norm 集合中取出对应的列，然后将这些值赋值给 Image_IG_mask 数组中的与超级补丁位置对应的区域
        Image_IG_mask[Superpatch_pos['Y'] * WSI_patch_dimension:(Superpatch_pos['Y'] + 1) * WSI_patch_dimension:,
        Superpatch_pos['X'] * WSI_patch_dimension:(Superpatch_pos['X'] + 1) * WSI_patch_dimension] = (edge_mask_norm[col_idx])

        # 将 Image_IG_mask_count 数组中与超级补丁位置对应的区域的元素设置为 1，以计算这个区域已经被处理的次数
        ...
        for Env_y, Env_x in zip(Envpatch_pos['Y'].tolist(), Envpatch_pos['X'].tolist()):
            ...
            # 通过 Envpatch_pos 字典提供的位置以及超级补丁的维度，将在 Image_mask 数组中指定的区域内的元素的值设置为 1
            Image_mask[Env_y * WSI_patch_dimension:(Env_y + 1) * WSI_patch_dimension,Env_x * WSI_patch_dimension:(Env_x + 1) * WSI_patch_dimension] = 1

            # 将 Image_IG_mask 数组中与环境补丁位置对应的区域与 edge_mask_norm[col_idx] 相加，然后将结果赋值给 Image_IG_mask 数组中与该位置对应的区域
            Image_IG_mask[Env_y * WSI_patch_dimension:(Env_y + 1) * WSI_patch_dimension,
            Env_x * WSI_patch_dimension:(Env_x + 1) * WSI_patch_dimension] = Image_IG_mask[Env_y * WSI_patch_dimension:(Env_y + 1) * WSI_patch_dimension,
                                                                             Env_x * WSI_patch_dimension:(Env_x + 1) * WSI_patch_dimension] + (edge_mask_norm[col_idx])

            # 将 Image_IG_mask_count 数组中与环境补丁位置对应的区域中的元素加 1，以计算这个区域已经被处理的次数
            Image_IG_mask_count[Env_y * WSI_patch_dimension:(Env_y + 1) * WSI_patch_dimension,
            Env_x * WSI_patch_dimension:(Env_x + 1) * WSI_patch_dimension] = Image_IG_mask_count[Env_y * WSI_patch_dimension:(Env_y + 1) * WSI_patch_dimension,
                                                                             Env_x * WSI_patch_dimension:(Env_x + 1) * WSI_patch_dimension] + 1
    #print("无效值： ", Image_IG_mask_count)
    Image_IG_mask = Image_IG_mask / Image_IG_mask_count
    median_value = np.nanmedian(Image_IG_mask)
    # 将大于中值的像素值减去 0.2，小于中值的像素值减去 0.3
    # Image_IG_mask[Image_IG_mask >= median_value] -= 0.1
    Image_IG_mask -= 0.15
    # norm = mcolors.Normalize(vmin=-1, vmax=1)
    # Image_IG_mask = norm(Image_IG_mask)
    Image_IG_mask = np.uint8(255 * cm.coolwarm(Image_IG_mask))  # 色彩变多
    # Image_IG_mask = np.uint8(255 * cm.coolwarm_r(Image_IG_mask)) # 颜色反转
    ...
    WSI_node_idx = Position_pd['Unnamed: 0'].tolist()  # list: 2460
    WSI_node_idx = [int(item) for item in WSI_node_idx]
    WSI_Location = Location_pd.iloc[WSI_node_idx]
    X_pos = WSI_Location['X'].tolist()
    #print("X_pos: ", X_pos) # list: 2460  [123,124,122,123,124,125....]
    Y_pos = WSI_Location['Y'].tolist()
    #print("Y_pos: ", Y_pos) # list: 2460  [0,0,1,1,1,,1....]
    X_Y_pos = [(X, Y) for X, Y in zip(X_pos, Y_pos)] # list: 2460   [(123,0),(124,0),(122,1),(123,1),(124,1)...]
    pos_dict = zip(Position_pd['Unnamed: 0.1'].tolist(), X_Y_pos)
    pos_dict = dict(pos_dict)
    # node 3 has no position
    ...
    if len(Mid_IG_subgraph) > 0:
        ...
        for c_item in Mid_IG_subgraph:
            ...
            if len(Mid_IG_subgraph_nodes) == 0:
                ...
                Mid_IG_subgraph_nodes = pd.concat([Mid_IG_subgraph_nodes, pd.DataFrame(subgraph_nodes).transpose()],
        ...
        if len(Mid_IG_subgraph_nodes) != 0:
            ...
            Mid_IG_subgraph_nodes.to_csv(os.path.join('/'.join(core_dir.split('/')[:-1]),
                                                      core_dir.split('/')[-1] + '_TME_subgraph_small_cap_subgraph_exist_again.csv'))
        else:
            print(Select_ID)
...
class CoxGraphDataset(Dataset):
    def __init__(self, filelist, survlist, stagelist, metadata, censorlist, mode, model, transform=None, pre_transform=None):#, metadata
    ...
    def get(self, idx):
        ...
        data_re = Data(x=data_origin.x[:,:1794], edge_index=data_origin.edge_index)
            ...
            if len(Whole_IG_value) == 0:
                ...
                Whole_IG_value = np.concatenate((Whole_IG_value,
                                                 np.load(os.path.join(patient_root,
                                                                  patient_features[0]))))
###
    threshould = np.median(Whole_IG_value)
    print(threshould)
    pos_term = np.where(Whole_IG_value >= threshould)[0]
    neg_term = np.where(Whole_IG_value < threshould)[0]
...
###
    pos_term = np.where(outlier_removed_IG_value >= threshould)[0]
    neg_term = np.where(outlier_removed_IG_value < threshould)[0]
...
def Calculate_IG(d, d_count, row, col, edge_mask, edge_mask_norm, patient_target_dir,
    ...
    a = d.absolute_path[d_count]
    # p_id = a.split('_')[0]
    p_id = a.split('\\')[-1].split('_')[0]
    # p_id = p_id.split('\\')
    # p_id = p_id[-1]
    id_path = d.absolute_path[d_count]
    id_path = id_path.split('\\')
    id_path = id_path[-1]

    #WSI_dir = './Sample_data_for_demo/Raw_WSI/'
    # WSI_dir = r"D:\PycharmProjects\zhongshan_hospital\Graph_neural_network\patch_construction\raw_data"
    WSI_dir = r"D:\PycharmProjects\zhongshan_hospital\Graph_neural_network\IG_analysis_kongzhuan\svs"
    print("Directory exists:", os.path.exists(WSI_dir))
    print("Directory contents:", os.listdir(WSI_dir))
    print("Absolute path:", os.path.abspath(WSI_dir))
    print("Directory permissions:", oct(os.stat(WSI_dir).st_mode))
    ...
    # Position_pd = pd.read_csv(os.path.join(dataset_root, id_path) + '_4.3_artifact_sophis_final.csv')
    Position_pd = pd.read_csv(os.path.join(r"D:\PycharmProjects\zhongshan_hospital\Graph_neural_network\IG_analysis_kongzhuan\graph", id_path) + '_4.3_artifact_sophis_final.csv')
    #print("sdasdasdasdasdasdasdasdada, ", id_path)   id_path = SelectID
    # Location_pd = pd.read_csv(
    #     os.path.join(dataset_root , '_'.join(id_path.split('_')[0:1])) + '_node_location_list.csv')
    Location_pd = pd.read_csv(os.path.join(r"D:\PycharmProjects\zhongshan_hospital\Graph_neural_network\IG_analysis_kongzhuan\graph" , '_'.join(id_path.split('_')[0:1])) + '_node_location_list.csv')
    Mask_fig_IG = new_Whole_supernode_vis(WSI, Position_pd, Location_pd, id_path, patient_target_dir, edge_mask_norm, row,
                                      col)
...
def Subgraph_analysis_vis(Argument):
    ...
    calculate_loader = DataLoader(TestDataset, batch_size=batch_num, shuffle=True, num_workers=1, pin_memory=False,
                                  drop_last=False)
        ...
        with torch.set_grad_enabled(False):
            for c, d in enumerate(calculate_loader, 1):
                ...
                for inside_count, (batch_item_num, adj_batch_item_num, id_item) in enumerate(
                    ...
                    id_item1 = id_item.split('\\')[-1]
                    patient_target_dir = os.path.join(coredir, id_item1)
