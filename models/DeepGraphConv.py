...
class DeepGraphConv_Surv(torch.nn.Module):
    ...
    def __init__(self, dropout_rate, dropedge_rate, Argument):
        ...
        self.path_attention_head = Attn_Net_Gated(L=hidden_dim * 5, D=hidden_dim * 5, dropout=dropout, n_classes=1)
        self.path_rho = nn.Sequential(*[nn.Linear(hidden_dim * 5, hidden_dim * 5), nn.ReLU(), nn.Dropout(dropout)])

        self.risk_prediction_layer = nn.Linear(hidden_dim * 5, 1)
